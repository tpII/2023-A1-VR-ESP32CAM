...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true)
    {
        ...
        if (res == ESP_OK)
        {
            ...
            String timestamp = getFormattedTimeWithMillis();

            snprintf(timestampHeader, sizeof(timestampHeader), "%s", timestamp.c_str());
            size_t hlen = snprintf((char *)part_buf, 128, _STREAM_PART, _jpg_buf_len, _timestamp.tv_sec, _timestamp.tv_usec, timestampHeader);
        ...
        //     char timestampHeader[64];
        //     String timestamp = getFormattedTimeWithMillis();
        //     snprintf(timestampHeader, sizeof(timestampHeader), "X-Timestampp: %s\r\n", timestamp);

        //     // Send the timestamp header
        //     res = httpd_resp_send_chunk(req, timestampHeader, strlen(timestampHeader));
        // }
